...
pacman::p_load(data.table, ggplot2,dplyr, stringr, udpipe, lattice, tidytext, readr, SnowballC, textstem, syuzhet, igraph, tidyr,RColorBrewer,wordcloud)
```
and now the data:
```{r}
...
pride_prejudice <- as.data.table(pride_prejudice_raw_text) 
...
pp_tf <- pride_prejudice[, unnest_tokens(.SD, output = word, input = text)
                         ][, .(n = .N), by = word
                         ][order(-n)]
pp_tf
```
